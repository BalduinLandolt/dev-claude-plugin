...
agents/
  ...
  coordinator/           Stateless implement-worker subagent (sonnet)
```
...
**Skill invocation and spawn depth**: Two transitions show up in this plugin, and they behave differently. (1) `Skill` invocations run the called skill body *in the caller's context* — no subagent boundary, no fresh window, no return value other than what the skill writes. They are zero-cost from a spawn-depth perspective. (2) `Agent` spawns create a real subagent in its own context window, and Claude Code currently only supports `Agent` reliably at **depth 1** (i.e., the agent doing the spawning must itself be in the user-facing main context, not inside another subagent). The orchestrator (`/dev:next`) uses `Skill` to call downstream skills (`/dev:plan`, `/dev:review-plan`, `/dev:implement`, `/dev:review-impl`) — all of those skill bodies execute in the orchestrator's window. Those skill bodies then `Agent`-spawn workers and reviewers, which is the single permitted depth-1 spawn. Concretely: `/dev:next` → `Skill /dev:implement` (no boundary) → `Agent dev:coordinator:implement-worker` (depth 1 ✓); `/dev:implement` → `Skill /dev:review-impl` (no boundary) → `Agent dev:review:*` (still depth 1 from the orchestrator ✓). Plan checkboxes, the issues journal, commit boundaries, and the review-checkpoint orchestration all live in the skill bodies, not in wrapper agents. Workers carry no `Agent`, `Skill`, or `AskUserQuestion` tool: blockers bubble up via the report and the skill body escalates if needed.
