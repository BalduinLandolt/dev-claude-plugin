...
**Skill invocation, Workflow, and spawn depth**: Three mechanisms move work around. (1) `Skill` invocations run the called skill body *in the caller's context* — no subagent boundary. The orchestrator uses this for the interactive phases (e.g. `plan`), and each phase-runner uses it to run its phase skill in its own context. (2) `Agent` spawns create a real subagent in its own context window: the orchestrator spawns the phase-runners — and, on the main-thread review path, reviewers — at depth 1; a phase-runner in turn spawns its workers at depth 2 (`implement` → `implement-worker`). Depth-5 nesting is available, so this composes. (3) The `Workflow` tool runs a deterministic script that fans out *its own* agents, but it is available only on the main thread — so `review-impl`/`review-plan` use it when the orchestrator invokes them (reviewer transcripts and the verification stay isolated; only compact findings return) and fall back to direct `Agent` spawns otherwise. Reviewer output accumulating in the orchestrator context is a profiled top cost driver — hence the fan-out and the code-fixes are pushed off the main thread. Workers carry no `Agent`, `Skill`, or `AskUserQuestion` tool: blockers bubble up via the report.

**Phase isolation**: the non-interactive phases run inside a `dev:coordinator:phase-runner` subagent (Sonnet) that invokes the phase skill via `Skill` in its own context and returns a compact summary, keeping that phase's transcript off the orchestrator thread. `implement` (worker loop + test-reviewer checkpoint + commits + docs), `learn` (which spawns the doc-improver at depth 2), and `prepare-pr` (git cleanup + PR) all run this way. The runner has no `AskUserQuestion`: it returns blockers for the orchestrator to escalate, and the phase skills resume from on-disk state when re-spawned. One split matters: the `Workflow` tool is available only on the main thread, so the `implement` *code-review* loop (`review-impl`) runs in the orchestrator after the driver returns — preserving the adversarial verify pass on code reviews while still isolating the coding. The interactive spine (`investigate`, `plan`, and all human gates) stays on the main thread, since a subagent cannot talk to the user.
