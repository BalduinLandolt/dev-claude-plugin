...
**Coordinator trace (post-hoc audit)**: Because the orchestrator only sees compact coordinator summaries, there is no built-in way to verify a coordinator followed its contract (which subagents it spawned, in what order, what each returned). The implement / review-plan / review-impl skill bodies append structural entries to `docs/design/plans/<task>/coordinator-trace.md` throughout a run. Skipped in minimal mode (no plan directory).

The review coordinators harden the trace's diagnostic value with three measures: (1) at the start of each run, they record a registry-check entry listing which `dev:review:*` agents they could see in their subagent catalogue, and if all are missing they abort with `Status: escalated` rather than falling through to inline review; (2) the review skill bodies log a one-line Summary excerpt for each clean reviewer alongside the round-collected entry, so every reviewer's return leaves per-reviewer evidence (non-clean ones via finding attribution, clean ones via the Summary excerpt); (3) before returning, the coordinators self-verify their summary against the trace, treating the trace as authoritative if the two disagree.

The trace is preserved by `/dev:learn` (committed alongside the PRD and plan) so future diagnostic questions like "did the reviewers actually spawn?" can be answered from git history. Worker-logs are still deleted, since those are bulky LLM-blob outputs and the journal captures the signal. Each coordinator summary also includes a "Run trace" section with broad-strokes bullets so the orchestrator and user see *what happened*, not just *what was delivered*. The trace is best-effort LLM self-reporting and defends against confused misreports, not adversarial fabrication; cross-check against commits, diffs, and file edits when verification matters.
